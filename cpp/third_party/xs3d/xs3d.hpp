// xs3d 1.13.0 (William Silversmith, LGPL-3.0-or-later), modified for Brook in 2026: thread_local scratch; see README.md.
#ifndef __XS3D_HPP__
#define __XS3D_HPP__
...
// Brook: per-thread state; released by the native call guard.
thread_local PersistentShapeManager persistent_data;
...
float robust_calc_area_at_point_2x2x2_persistent_data(
	...
	const std::vector<float>& projections, 
	const std::vector<float>& inv_projections,
	std::vector<uint8_t>& visited, const uint8_t color
) {
...
std::tuple<float, uint8_t> cross_sectional_area_helper_2x2x2_persistent_data(
	...
	const uint8_t shape_color = persistent_data.next_color();
	auto &shape_visited = persistent_data.visited;
	...
	while (!stack.empty()) {
		...
		total += robust_calc_area_at_point_2x2x2_persistent_data(
			...
			sx, sy, sz,
			cur, pos, normal, anisotropy,
			pts, projections, inv_projections, shape_visited, shape_color
		);
	}
